...
def show_accuracy(a, b):
    acc = a.ravel() == b.ravel()
    print('正确率：%.2f%%' % (100*float(acc.sum()) / a.size))#正确率


if __name__ == "__main__":
    data = np.loadtxt('14.bipartition.txt', dtype=np.float, delimiter='\t')
    x, y = np.split(data, (2, ), axis=1)# 第一个参数是要操作的数组对象，第二个参数是将n维(n行)数组
                                        # 分片，其中前两列作为第一个分片，后面几列作为第二个分片
                                        # x 表示训练集属性
    y[y == 0] = -1
    y = y.ravel()                       # 将二维数组y降为一维，y=0改成y=-1,表示两种分类
    ...
    x1, x2 = np.mgrid[x1_min:x1_max:500j, x2_min:x2_max:500j]  # 生成网格采样点,每一行各生成500个相同的点，
                                                               # 第一行是列最小点，最后一行是列最大点，生成
                                                               # 两个500*500的二维矩阵，对第一个矩阵来说，
                                                               # 行元素相同，第二个矩阵列元素相同。
   
    grid_test = np.stack((x1.flat, x2.flat), axis=1)  # 测试点,flat表示将二维矩阵降为一维数组，stack合并，
                                                      # axis=0表示按行合并，就是两行分块，axis=1表示按列合并
                                                      # 就是每行两个元素，原来有多少列现在就有多少行，
                                                      # 得到500个样本数据
    print(grid_test)
